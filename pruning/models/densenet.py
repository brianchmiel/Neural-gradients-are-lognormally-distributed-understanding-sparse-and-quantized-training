...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        from .modules.prunning import ZeroBN,Conv2dStats,ReLUStats


        all_convs = [n for n, m in self.named_modules() if isinstance(m, Conv2dStats)]
        all_relu =  [n for n, m in self.named_modules() if isinstance(m, ReLUStats)]

        all_bn = [n for n, m in self.named_modules() if isinstance(m, ZeroBN)]
        self.stats_layers = all_convs + all_relu + all_bn
        self._pre_process_container(self)
    ...
    def _pre_process_container(self, container, prefix=''):
        ...
        for name, module in container.named_children():
            ...
            if full_name in self.stats_layers:
                module.fullName = full_name
...
def densenet(**config):
    ...
    if config.pop('zeroBN', False):
        from .modules.prunning import ZeroBN, Conv2dStats
        torch.nn.Conv2d = Conv2dStats
        torch.nn.BatchNorm2d = ZeroBN
